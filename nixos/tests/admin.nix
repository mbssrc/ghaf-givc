          ...
          testScript =
            ...
            ''
              ...
              with subtest("test user notification (local)"):
                guivm.succeed("(timeout 2 busctl --user --machine=ghaf@ monitor org.freedesktop.Notifications > /tmp/notification.log 2>&1 &)")
                time.sleep(1)

                # Send test notification directly to local socket
                guivm.succeed("echo '{\"event\":\"some-alert\",\"title\":\"Local Test Alert\",\"criticality\":\"critical\",\"message\":\"This is a test notification\"}' | socat - UNIX-CONNECT:${userSocket}")
                time.sleep(1)
              ...
              with subtest("test user notification (remote)"):
                ...
                appvm.succeed("${cli} ${cliArgs} notify-user gui-vm --event vm-alert --title 'VM Test Alert' --criticality 'critical' --message 'This is a VM test notification' ")
                time.sleep(1)
